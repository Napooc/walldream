  ...
  return (
    <motion.nav
      ...
      className={`fixed top-0 left-0 right-0 z-50 transition-all duration-300 ${
        isScrolled ? "backdrop-blur-xl shadow-2xl border-b border-border/50" : ""
      }`}
    >
      <div className="container mx-auto px-6">
        <div className="flex items-center justify-between h-20">
          ...
          >
            <Button
              variant="ghost"
              className={`rounded-full ${isScrolled ? "text-foreground" : "text-white hover:bg-white/20"}`}
            >
              Connexion
            </Button>
          ...
          <button
            ...
            className={`lg:hidden p-2 rounded-xl ${isScrolled ? "text-foreground" : "text-white"}`}
          >
