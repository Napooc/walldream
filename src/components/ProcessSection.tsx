...
const ProcessSection = () => {
  ...
  return (
    <section ref={sectionRef} className="py-32 bg-gradient-to-b from-background to-muted/50 relative overflow-hidden">
      <motion.div 
        style={{ y }}
        className="absolute right-0 top-1/2 -translate-y-1/2 w-1/2 h-[600px] hidden lg:block"
      >
        <div className="relative w-full h-full rounded-l-[3rem] overflow-hidden shadow-2xl">
      ...
      <div className="container mx-auto px-6 relative z-10">
        ...
        >
          <div className="inline-block mb-6 px-6 py-3 bg-gradient-to-r from-primary/20 to-secondary/20 rounded-full">
            <span className="text-primary font-semibold">NOTRE PROCESSUS</span>
          </div>
              ...
              >
                <div className="flex gap-6 items-start">
                  <div className="relative flex-shrink-0">
                    ...
                    <div className="relative w-20 h-20 rounded-2xl bg-gradient-to-br from-card to-muted border-2 border-border flex items-center justify-center group-hover:scale-110 transition-transform duration-300">
                      <Icon className="w-10 h-10 text-foreground" />
                    </div>
                    <div className="absolute -bottom-2 -right-2 w-8 h-8 bg-secondary rounded-full flex items-center justify-center text-white font-bold text-sm">
                      {index + 1}
                    </div>
                  </div>
                ...
                {index < steps.length - 1 && (
                  <div className="ml-10 mt-4 mb-4 h-12 w-0.5 bg-gradient-to-b from-border to-transparent" />
                )}
              </motion.div>
            );
          })}
